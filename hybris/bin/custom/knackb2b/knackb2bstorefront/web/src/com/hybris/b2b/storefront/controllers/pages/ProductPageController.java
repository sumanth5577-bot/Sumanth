 ...
 */
package com.hybris.b2b.storefront.controllers.pages;
...
import com.hybris.b2b.storefront.controllers.ControllerConstants;
...
public class ProductPageController extends AbstractPageController
{
	...
	{
		final ProductModel productModel = productService.getProductForCode(productCode);
		productModel.getExtMediaImgUrl();
		getRequestContextData(request).setProduct(productModel);
		...
		sortVariantOptionData(productData);
		storeCmsPageInModel(model, getPageForProduct(productCode));
		// Bharath - Set the product url.
		productData.setExtMediaImgUrlList(productModel.getExtMediaImgUrl());
		populateProductData(productData, model);
	...
	{
		if (productData.getExtMediaImgUrlList() != null && productData.getExtMediaImgUrlList().size() > 0)
		{
			//todo
			model.addAttribute("extMediaImgUrl", productData.getExtMediaImgUrlList());
		}
	...
	{//Bharath to modify gallery images
		final List<Map<String, ImageData>> galleryImages = new ArrayList<>();
		if (productData.getExtMediaImgUrlList() != null && productData.getExtMediaImgUrlList().size() > 0)
		{
			// todo nothing
		}
		else if (CollectionUtils.isNotEmpty(productData.getImages()))
		{
